using Microsoft.Extensions.Configuration; //Didnt auto find had to add myself for configuration
...
namespace HiddenVilla_Api.Controllers
{
    ...
    public class StripePaymentController : Controller
    {
        ...
        public async Task<IActionResult> Create(StripePaymentDTO payment)
        {
            try
            {
                //var domain = _configuration.GetSection("HiddenVilla_Client_URL").Value.ToString();
                var domain = _configuration.GetValue<string>("HiddenVilla_Client_URL");
                var options = new SessionCreateOptions //using checkout and not billing portal
                {
